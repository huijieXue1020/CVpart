...
# 全局参数
# audio_dir = 'C:\\Users\\w1792\\Desktop\\代码\\任务5.老人员工义工人脸图像采集\\audios'

image_dir = 'C:\\Users\\w1792\\Desktop\\laorenyouhao\\A_Final_Sys\\images\\volunteers'
id = '102'
...
# 新建目录
if os.path.exists(os.path.join(image_dir, id)):  #文件是否存在
    shutil.rmtree(os.path.join(image_dir, id), True) #递归删除文件
os.mkdir(os.path.join(image_dir, id))    #创建目录

# 开始采集人脸
for action in action_list:
    # audioplayer.play_audio(os.path.join(audio_dir, action + '.mp3'))
    action_name = action_map[action].encode().decode()
    ...
    for i in range(15):
        ...
        draw.text((int(image.shape[1] / 2), 30), action_name,
                  ...
                  # 'NotoSansCJK-Black.ttc', 40   "C:\\WINDOWS\\Fonts\\simhei.ttf"
                  fill=(255, 0, 0))  # linux
